...
void CombatScreen::AITurn(Enemy* enemy) {
...
void CombatScreen::DoAction(Action* action) {
	bDoingAction = true;
...
void CombatScreen::MouseClicked() {
...
void CombatScreen::InitCombatGrid() {
...
void CombatScreen::SortInitiative(vector<Actor*> units){
...
void CombatScreen::Update(Time t) {
	...
	if (enemyDeadCount == enemyUnits.size()) {
		bVictory = true;
	}
	if (playerDeadCount == playerUnits.size()) {
		bLoss = true;
	}
